...
export const createCartProductElement = ({ id, title, price, pictures }) => {
  ...
  li.addEventListener('click', () => {
    ...
    const convertToNumber = Number(totalPrice.innerText);

    removeCartProduct(li, id);

    const total = convertToNumber - price;
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', () => {
    ...
    const converterNumero = Number(valorTotal.innerHTML);
    const addProdutoNoCarrinho = async () => {
      saveCartID(id);
      const produto = await fetchProduct(id);
      const total = converterNumero + produto.price;
      valorTotal.innerHTML = total.toFixed(2);
      const idProduto = createCartProductElement(produto);
